...
Usage:
    ...
    python run.py --prefer kinect     # try a Kinect first, then fall back
...
def main() -> None:
    ...
    p.add_argument("--prefer", choices=["auto", "kinect", "webcam"], default=None,
                   help="which kind of camera to try first (overrides the saved setting)")
    p.add_argument("--host", default="127.0.0.1")
    p.add_argument("--port", type=int, default=8000)
    args = p.parse_args()

    force = args.sensor or ("mock" if args.mock else None)
    backend_mode = args.prefer
